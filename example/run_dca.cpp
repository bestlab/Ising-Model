...
void init_default(model &mymodel, arma::mat &msa_freq){
  ...
  for(int i=0; i<mymodel.N; i++){
    mymodel.h(i,0) = -1*log(msa_freq(i)+0.0001); //Small regularization to avoid divergence
  }
}

void fit(model &mymodel, arma::mat &msa_freq, arma::mat &msa_corr, int nr){
  ...
  if(file.is_open()){
    ...
    while(std::getline(file,line)){
      ...
      if(id=="J"){
        ...
        my_stream >> j;
        int index = (mymodel.N-1)*i-i*(i+1)/2+j-1;
        my_stream >> mymodel.J(index,0);
      }
      else if(id=="h"){
        int i;
        my_stream >> i;
        my_stream >> mymodel.h(i,0);
      }
      else{
...
void write_params(model &mymodel, std::string name){
  ...
  for(int i=0; i<mymodel.N-1; i++){
    for(int j=i+1; j<mymodel.N; j++){
      int index = (mymodel.N-1)*i-i*(i+1)/2+j-1;
      ofile << "J " << i << " " << j << " " << mymodel.J(index,0) << std::endl;
    }
  }

  for(int i=0; i<mymodel.N; i++){
    ofile << "h " << i << " " << mymodel.h(i,0) << std::endl;
  }
  ofile.close();
}
