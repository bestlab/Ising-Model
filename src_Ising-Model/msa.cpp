...
void lowmem_fill_freq(std::string msafile, arma::vec &msa_freq, arma::vec &msa_corr, int N){
...
void fill_freq(std::vector<int> &msa_seqs, std::vector<double> &weights,
		arma::vec &msa_freq, arma::vec &msa_corr,int nseq, bool symon){
  ...
  for(int m=0; m<nseq; m++){
    ...
    for(int i=0; i<N; i++){
      msa_freq(msa_seqs[m*N+i]) += weights[m];
    }
    for(int i=0; i<(N-1); i++){
      for(int j=(i+1); j<N; j++){
        int index = (N-1)*i-i*(i+1)/2+j-1;
        msa_corr(index) += weights[m];
      }
    }
  }
